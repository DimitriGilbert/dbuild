...
generate_bashticle() {
  ...
  # Generate the parseArger CLI
  if ! command -v parseArger &> /dev/null; then
    echo "Error: parseArger not found. Please ensure parseArger is installed and in your PATH."
    exit 1
  fi

  local cli_tmp=$(mktemp)
  parseArger generate \
    --output-file "$cli_tmp" \
    --pos 'step "Section to jump to" --optional --complete-custom "echo '"$section_list"'"' \
    --flag 'run-command "Execute commands instead of echoing" --short r' \
    --flag 'dry-run "Echo commands only (default)" --short d --on' \
    --flag 'stream "Enable streaming output" --short s --on' \
    --flag 'verbose "Verbose output" --short v' \
    --help-message "$title - An interactive bashticle" \
    --version-opt --set-version "1.0.0"

  # Copy CLI to output file and replace title
  sed "s/TITLE_PLACEHOLDER/$title/g" "$cli_tmp" > "$output_file"
  rm -f "$cli_tmp"
